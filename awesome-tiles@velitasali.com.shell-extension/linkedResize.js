...
export class LinkedResizeHandler {
  ...
  enable() {
    ...
    this._modifierSettingsId = this._settings.connect('changed::shortcut-linked-resize', () => {
      this._updateModifierMask()
    })
  }
  ...
  disable() {
    ...
    if (this._modifierSettingsId) {
      this._settings.disconnect(this._modifierSettingsId)
      this._modifierSettingsId = null
    }
  ...
  _updateModifierMask() {
    const shortcuts = this._settings.get_strv('shortcut-linked-resize')
    ...
    if (shortcut.includes('control') || shortcut.includes('ctrl')) mask |= Clutter.ModifierType.CONTROL_MASK
    if (shortcut.includes('shift')) mask |= Clutter.ModifierType.SHIFT_MASK

    this._modifierMask = mask || Clutter.ModifierType.MOD1_MASK
  }
  ...
  _onCapturedEvent(event) {
    ...
    if (type === Clutter.EventType.KEY_PRESS || type === Clutter.EventType.KEY_RELEASE) {
      ...
      const isModifierEvent = (mods & this._modifierMask) !== 0 || this._isEventModifier(event)
      
      if (isModifierEvent) {
        this._altKeyPressed = isPressed
        if (!isPressed && this._isResizing) {
          this._stopResize()
        }
      }
    }
  ...
  _isEventModifier(event) {
    const symbol = event.get_key_symbol()
    if (this._modifierMask & Clutter.ModifierType.MOD1_MASK) {
      if (symbol === Clutter.KEY_Alt_L || symbol === Clutter.KEY_Alt_R) return true
    }
    if (this._modifierMask & Clutter.ModifierType.SUPER_MASK) {
      if (symbol === Clutter.KEY_Super_L || symbol === Clutter.KEY_Super_R) return true
    }
    if (this._modifierMask & Clutter.ModifierType.CONTROL_MASK) {
      if (symbol === Clutter.KEY_Control_L || symbol === Clutter.KEY_Control_R) return true
    }
    if (this._modifierMask & Clutter.ModifierType.SHIFT_MASK) {
  ...
  _onGrabOpBegin(window, grabOp) {
    ...
    const isModifierHeld = (mods & this._modifierMask) !== 0
